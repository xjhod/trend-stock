...
def weekly_trend_up(wk):
    if len(wk) < 21: return None
    closes = [w["close"] for w in wk]
    ma5 = sum(closes[-5:])/5; ma10 = sum(closes[-10:])/10; ma20 = sum(closes[-20:])/20
    c = closes[-1]
    return (ma5 > ma10 > ma20) and (c > ma10)

def weekly_up_retrace(wk):
    if len(wk) < 21: return None
    closes = [w["close"] for w in wk]
    ma5 = sum(closes[-5:])/5; ma10 = sum(closes[-10:])/10; ma20 = sum(closes[-20:])/20
    c = closes[-1]
    if not (ma5 > ma10 > ma20 and c > ma10): return None
    hi20 = max(w["high"] for w in wk[-20:])
    dd = (1 - c/hi20) * 100 if hi20 else 99
    if dd < 3 or dd > 20: return None
    return True

mkt_dates = [r["date"] for r in json.load(open(os.path.join(BASE,"research","mkt_long.json"), encoding="utf-8")) if "2023-01-01" <= r["date"] <= "2026-08-28"]
SCAN = mkt_dates[::5]
...
for k, asof in enumerate(SCAN):
    for it in pool:
        ...
        if not wmap or len(wmap) < 21: continue
        dates_w = [w["date"] for w in wmap]
        idx = bisect.bisect_right(dates_w, asof)
        if idx < 21: continue
        wk = wmap[:idx]
        r1 = weekly_trend_up(wk); r2 = weekly_up_retrace(wk)
    ...
    if (k+1) % 60 == 0: print(f"  {k+1}/{len(SCAN)} W1={len(records['W1'])} W2={len(records['W2'])}", flush=True)
...
def rep(t, rs):
    ...
    print(f"  {t:<30} n={len(rs):<4} "+"  ".join(parts))

print("\n===== 周线真趋势结果 =====")
rep("W1 周线多头排列", records["W1"])
rep("W2 周线多头+回调3-20%", records["W2"])
